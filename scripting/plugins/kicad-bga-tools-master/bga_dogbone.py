...
def make_dogbone(board, mod, bga_info, skip_outer, edge_layers):
    ...
    via_dia=design.GetCurrentViaSize()
    isolation = design.GetSmallestClearanceValue()
...
class menu(ActionPlugin):
    def defaults( self ):
        ...
        self.icon_file_name = path.join(path.dirname(__file__), 'bg_add.png')
